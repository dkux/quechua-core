...
public class AdministradorResource {
    ...
    private final ProfesorResource profesorResource;

    public AdministradorResource(UserService userService, AlumnoService alumnoService, ProfesorResource profesorResource,
                                 AuthorityRepository authorityRepository, AlumnoCarreraService alumnoCarreraService,
                                 CarreraRepository carreraRepository) {
        this.userService = userService;
        this.authorityRepository = authorityRepository;
        this.alumnoService = alumnoService;
        this.profesorResource = profesorResource;
    ...
    @Timed
    public CargaMasivaDTO createAlumnoCarrera(@RequestParam("file") MultipartFile file) {
            ...
            while ((line = reader.readNext()) != null) {
                ...
                if (userOptional.isPresent()) {
                    ...
                    if (userOptional.get().getAuthorities().contains(authorityRepository.findById(AuthoritiesConstants.ALUMNO).get())) {
                        cargaMasivaDTO.getWarningMessages().add(String.format("%s ya existia como alumno", email));
                    } else {
                        UserDTO dto = new UserDTO(userOptional.get());
                        dto.getAuthorities().add(AuthoritiesConstants.ALUMNO);
                        userService.updateUser(dto);
                        cargaMasivaDTO.getWarningMessages().add(String.format("%s ya existia y se le asignó el rol de alumno", email));
    ...
    private boolean isValidAlumnoRow(String nombre, String apellido, String email, String padron, String prioridad,
        ...
        if (!tieneCarreraValida) {
            cargaMasivaDTO.getErrorMessages().add(String.format("Línea %d no tiene carreras válidas", lineNumber, prioridad));
            hasErrors = true;
        }
        return !hasErrors;
    }
